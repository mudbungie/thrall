...
#
# PORTED FROM LITANY (its bl-f963), under yog `docs/DESIGN.md` §10.1. The copy
# is deliberately close to byte-identical, for the same reason `leak-scan.sh`
# here is a close copy of yog's: two copies of a disclosure gate drift within a
# week, and a short `diff` against the sibling is the only cheap defence. Every
# deliberate divergence is commented where it sits. §10.1 is explicit that each
# repo owns its own image tooling and that there is no shared build system —
...
# operator-issued leaf material and a config of argv commands, and the
# `Containerfile` says in as many words that a certificate baked into a layer
# is a certificate published to everyone who can pull it (DESIGN §3.2, §3.3).
# Until this file, nothing checked that sentence.
#
...
# needs. Nothing below uses an associative array, `mapfile`, or `readlink -f`
# outside the guarded idiom the other scripts already use.
#
