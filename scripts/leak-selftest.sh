# thrall leak scan — the regression half, and the reason the gate cannot rot.
# Ported from yog with the scanner (bl-e878); one case is dropped, and the
...
#
# A `grep -q` HERE READS FROM A HERESTRING, NEVER FROM A PIPE (bl-da2a), and the
# check at the foot of `self_test` holds every tracked bash script in this repo
# to it. A `grep -q` on the receiving end of a pipe is a race, not a style: it
# exits the instant it matches and closes the read end, the writer is killed by
...
# in silence — the gate lying rather than a harness mis-scoring itself. It is a
# flake only while the subject fits one write: a writer that outruns the pipe
# buffer answered falsely 300 times in 300. The ban is on the SHAPE rather than
# on the option, because a sourced file cannot see whether its caller set
# `pipefail` — this one does not set it and inherits it, which is how the defect
# reached the one file whose whole job is to prove the gate is not lying — and a
...
self_test() {
  ...
  # thrall's allowlist is empty (`^$` — see leak-rules.sh), so there is no such
  ...
  # pipeline reports is the answer its reader gave.
  #
  # SCOPE IS WHERE `pipefail` EXISTS, which is bash. A `#!` naming any other
  # interpreter is skipped — `scripts/thrall-leak-gate` is POSIX `/bin/sh` on
